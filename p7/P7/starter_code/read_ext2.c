...
int read_group_descs(int fd,				  /* the disk image file descriptor */
					 ext2_group_desc *groups, /* where to put the group-descriptor table; it takes one page */
...
void read_inode(fd, offset, inode_no, inode, s_inode_size) int fd; /* the floppy disk file descriptor */
